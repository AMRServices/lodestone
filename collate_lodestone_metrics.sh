...
echo -e "RunID\tSampleID\tRawReads\tHQReads\tRawReadLen(Avg)\tHQReadLen(Avg)\tRawReadQual(Avg)\tHQReadQual(Avg)\tKaiju_PE_TopHit\tKaiju_PE_SecHit\tCandidateTaxon\tCandidateTaxon_NCBITaxID\tNumGenomeDwl\tMASH_ClosestRef\tMASH_ClosestRef_AccID\tBWA_ReadsMapped(%)\tGenomeCoverage(%)\tDepthofCoverage(X)\tSNPs\tHQ_SNPs(QUAL>=30, DP>=20)\tNumContigs\tNumContigs(>=1kb)\tTotalLength\tGC(%)\tN50\tLodestoneError\tLodestone_TotalProcTime(min:sec)" > ${OUTFILE}

echo -e "${RUNID}\t${CAVID}\t${rawreads_for}|${rawreads_rev}\t${hqreads_for}|${hqreads_rev}|${hqreads_se}\t${rawreads_len_for}|${rawreads_len_rev}\t${hqreads_len_for}|${hqreads_len_rev}|${hqreads_len_se}\t${rawreads_qual_for}|${rawreads_qual_rev}\t${hqreads_qual_for}|${hqreads_qual_rev}|${hqreads_qual_se}\t${top_sp_perc_pe}% ${top_sp_pe}\t${sec_sp_perc_pe}% ${sec_sp_pe}\t${taxname}\t${taxid}\t${num_genomes}\t${mash_closest_ref_str}\t${mash_closest_ref_acc}\t${perc_mapped}\t${perc_genome_covered}%\t${coverage}\t${snps}\t${hqsnps}\t${num_contigs}\t${num_contigs_1kb}\t${tot_len}\t${gc}\t${n50}\t${err}\t${proctime}" >> ${OUTFILE}
