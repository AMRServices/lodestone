...
when SNPs are called against a fixed reference 
...
echo -e "RunID\tSampleID\tRawReads\tHQReads\tRawReadLen(Avg)\tHQReadLen(Avg)\tRawReadQual(Avg)\tHQReadQual(Avg)\tBWA_ReadsMapped(%)\tGenomePositionCovered\tGenomeCoverage(%)\tDepthofCoverage(X)\tSNPs\tHQ_SNPs(QUAL>=30, DP>=20)\tNumContigs\tNumContigs(>=1kb)\tTotalLength\tGC(%)\tN50\tLodestoneError\tLodestone_TotalProcTime(min:sec)" > ${OUTFILE}

echo -e "${RUNID}\t${CAVID}\t${rawreads_for}|${rawreads_rev}\t${hqreads_for}|${hqreads_rev}|${hqreads_se}\t${rawreads_len_for}|${rawreads_len_rev}\t${hqreads_len_for}|${hqreads_len_rev}|${hqreads_len_se}\t${rawreads_qual_for}|${rawreads_qual_rev}\t${hqreads_qual_for}|${hqreads_qual_rev}|${hqreads_qual_se}\t${perc_mapped}\t${nonzero_pos}\t${perc_genome_covered}%\t${coverage}\t${snps}\t${hqsnps}\t${num_contigs}\t${num_contigs_1kb}\t${tot_len}\t${gc}\t${n50}\t${err}\t${proctime}" >> ${OUTFILE}
